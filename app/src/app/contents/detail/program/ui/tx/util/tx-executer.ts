import TxCommitRunner from './tx-commit-runner';
  ...
  export const run = async (props: {
    ...
    for (const row of rows) {
      ...
      switch (order.type) {
        ...
        case 'copy_file':
          ...
          break;
        case 'create_file':
          status.verify = await TxVerifyUtil.checkSaveFile(
            order.path,
            reserveDirs,
          );
          break;
    ...
    for (const row of rows) {
      ...
      switch (order.type) {
        case 'create_dir':
          {
            status.commit = await TxCommitRunner.makeDir(order.path);
          }
          break;
        case 'copy_file':
          {
            status.commit = await TxCommitRunner.copyFile(order.from, order.to);
          }
          break;
        case 'create_file':
          {
        ...
        case 'rename_file': {
          status.commit = await TxCommitRunner.renameFile(order.from, order.to);
          break;
        }
      }
