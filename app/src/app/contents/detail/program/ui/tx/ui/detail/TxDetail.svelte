<script lang="ts">
  ...
  export let detail: TxDetailUtil.Props;
...
<div class="cover">
  <Wrap margin={8}>
    <RecordDiv height={30}>
      <div class="header">
        <OperationButton name={'Close'} width={100} callback={close} />
      </div>
    </RecordDiv>
    <RecordDiv surplus={30}>
      <div class="main">
        {#if detail.kind === 'single'}
          <SingleTextViewer content={detail.content} />
        {:else if detail.kind === 'diff'}
          <DiffViewer original={detail.original} current={detail.current} />
        {/if}
      </div>
    </RecordDiv>
  </Wrap>
...
<style>
  ...
  .header {
    ...
    width: 100%;
    height: 100%;
    background-color: rgba(90, 90, 90, 0.999);
  }
  .main {
    display: inline-block;
    position: relative;
