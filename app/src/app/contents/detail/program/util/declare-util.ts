...
namespace DeclareUtil {
  ...
  export const createUtilObject = (
    ...
    switch (name) {
      case 'print':
        return createAppendTextCallback(RuntimeUtil.PLAIN_CHANNEL_ID, false);
      case 'println':
        return createAppendTextCallback(RuntimeUtil.PLAIN_CHANNEL_ID, true);
      case 'channel':
        return DclChannel.getObject(
          workerCache,
          createAppendTextCallback,
          createAppendTableCallback,
        );
      case 'fs': {
        return DclFileSystem.getObject(workerCache);
      }
      case 'runtime':
        return DclRuntime.getObject();
      case 'net':
        return DclNet.getObject();
      case 'state':
        return DclState.getObject(workerCache);
      case 'parser':
        return DclParser.getObject(rustCache);
    }
  };

  export const createUtilDeclareDef = (
    name: ReserveDef,
  ): {
    typeDec: string;
    valueDec: string;
